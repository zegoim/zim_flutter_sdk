...
static const BYTE aes_sbox[16][16] = {{0x63, 0x7C, 0x77, 0x7B, 0xF2, 0x6B, 0x6F, 0xC5, 0x30, 0x01,
...
static const BYTE aes_invsbox[16][16] = {{0x52, 0x09, 0x6A, 0xD5, 0x30, 0x36, 0xA5, 0x38, 0xBF,
...
* AES - CBC
*******************/
int aes_encrypt_cbc(const BYTE in[], size_t in_len, BYTE out[], const AES_WORD key[], int keysize,
                    const BYTE iv[]) {
...
int aes_encrypt_cbc_mac(const BYTE in[], size_t in_len, BYTE out[], const AES_WORD key[],
...
void aes_encrypt_ctr(const BYTE in[], size_t in_len, BYTE out[], const AES_WORD key[], int keysize,
                     const BYTE iv[]) {
...
void aes_decrypt_ctr(const BYTE in[], size_t in_len, BYTE out[], const AES_WORD key[], int keysize,
                     const BYTE iv[]) {
...
int aes_encrypt_ccm(const BYTE payload[], AES_WORD payload_len, const BYTE assoc[],
                    unsigned short assoc_len, const BYTE nonce[], unsigned short nonce_len,
                    BYTE out[], AES_WORD *out_len, AES_WORD mac_len, const BYTE key_str[],
                    int keysize) {
...
int aes_decrypt_ccm(const BYTE ciphertext[], AES_WORD ciphertext_len, const BYTE assoc[],
                    unsigned short assoc_len, const BYTE nonce[], unsigned short nonce_len,
                    BYTE plaintext[], AES_WORD *plaintext_len, AES_WORD mac_len, int *mac_auth,
    ...
    buf = (BYTE *)malloc(assoc_len + ciphertext_len /*ciphertext_len = plaintext_len + mac_len*/ +
                         48);
    if (!buf)
        return (FALSE);
...
void aes_key_setup(const BYTE key[], AES_WORD w[], int keysize) {
    int Nb = 4, Nr, Nk, idx;
    AES_WORD temp, Rcon[] = {0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000,
