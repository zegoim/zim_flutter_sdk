...
public class ZegoZimPlugin implements FlutterPlugin, MethodCallHandler, EventChannel.StreamHandler {
    ...
    public void onAttachedToEngine(@NonNull FlutterPluginBinding flutterPluginBinding) {
        methodChannel =
            new MethodChannel(flutterPluginBinding.getBinaryMessenger(), "zego_zim_plugin");
        methodChannel.setMethodCallHandler(this);

        EventChannel eventChannel =
            new EventChannel(flutterPluginBinding.getBinaryMessenger(), "zim_event_handler");
    ...
    public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
        try {
            ...
            if (method == null) {
                if (call.method.equals("create")) {
                    method = this.manager.getMethod(call.method, MethodCall.class, Result.class,
                                                    FlutterPluginBinding.class,
                                                    ZIMPluginEventHandler.class);
                } else {
                    method = this.manager.getMethod(call.method, MethodCall.class, Result.class);
                }
            ...
            pw.close();
            result.error("1",
                         "InvocationTargetException:" + e.getMessage() + "stackTrace:" + stackTrace,
                         null);
            ...
            pw.close();
            result.error(
                "1", "IllegalAccessException:" + e.getMessage() + "stackTrace:" + stackTrace, null);
        }
    }
